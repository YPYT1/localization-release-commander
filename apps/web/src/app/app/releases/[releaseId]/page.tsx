...
import { AddAssetForm, QueueReleaseButton, RunReleaseButton } from "@/components/forms";
...
export default async function ReleaseDetailPage({ params }: { params: Promise<{ releaseId: string }> }) {
  ...
  const [releaseResult, timelineResult, identity] = await Promise.all([api.release(releaseId), api.timeline(releaseId), api.me()]);
