...
import { ConnectionNotice, RoleNotice } from "@/components/data-states";
import { StatusBadge } from "@/components/status-badge";
import { api } from "@/lib/api";

export default async function SettingsPage() {
  const identity = await api.me();
  if (!identity.ok) return <ConnectionNotice result={identity} />;
  if (!identity.data.roles.includes("Admin")) return <><WorkspaceHeading eyebrow="SETTINGS / SERVER READ MODEL" title="工作区设置" detail="成员、平台连接与审计保留策略。" /><RoleNotice role="Admin" /></>;
